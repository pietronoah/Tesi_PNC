                           ...
                           Number* values)
{
  if (values == NULL) {
    ...
    auto* values2 = new double[nele_jac]();
    ...
    pattern_value_match_constrains_jacobian(sparsity_pattern_g_jac, values1, values2, m);
                       ...
                       Index* jCol, Number* values)
{
  if (values == NULL) {
    ...
    std::vector<long long int> obj_f_hes_sparsity_vect(obj_f_hes_sparsity, obj_f_hes_sparsity + (2+n+1+nele_hess));
