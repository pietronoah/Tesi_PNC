                           ...
                           Number* values)
{
  if (values == NULL) {
    ...
    const Number mult = 1;
    pattern_value_match_hessian(sparsity_pattern_g_jac,values1,sparsity_pattern_g_jac,final_values,1,&mult);

      std::cout << "Before: ";
      print_array(final_values,15);
    int final_index = 0;
    for (int i = 0; i < (n*m); i++) {
        if (final_values[i] != 0) {
    ...
    /*auto* values2 = new double[n*m]();
    auto* b = (long long int*) sparsity_pattern;
    pattern_value_match(values1, sparsity_pattern, values2);

    std::cout << "Pattern: ";
    print_array_long(b,23);

    std::cout << "Before: ";
    print_array(values2,15);
    int final_index = 0;
    for (int i = 0; i < (n*m); i++) {
        if (values2[i] != 0) {
            values[final_index] = values2[i];
            final_index++;
        }
    }
    std::cout << "After: ";
                       ...
                       Index* jCol, Number* values)
{
  if (values == NULL) {
    ...
    Index idx = final_pattern[2+n]; // RIGUARDARE QUI!!
    ...
    long long int nze_final = final_pattern[2+n]; // Number of non-zero elements in the final pattern
    auto* final_values = new double[nze_final]();
