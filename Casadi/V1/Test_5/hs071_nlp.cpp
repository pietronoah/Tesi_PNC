...
// CAMBIARE IL NOME DI QUESTA FUNZIONE PERCHÈ VIENE UTIIZZATA ANCHE NEL CALCOLO DELLA MATRICE HESSIANA
...
// Utilizzata nel merge di due mattern nella funzione per ricavare l'hessiano
...
/*void pattern_value_match_hessian(std::vector<long long int> short_patt, double* short_value, std::vector<long long int> long_patt, double* long_value, int n_cons, const Number* multiplier) {
    for(int q = 0; q < 2; q++) { // Loop over different constrains
...
/*void compact_constrain_values(std::vector<long long int> sparsity_pattern, double* values, double* final_values) {
    int n_row = (int) sparsity_pattern[0];
    int n_col = (int) sparsity_pattern[1];
    int n_cons = n_row/n_col;
    for(int q = 0; q < n_cons; q++) { // Loop over different constrains
        int index = 0;

        for(int i = 0; i < n_col; i++) {
            int nze = (int) (sparsity_pattern[3+i] - sparsity_pattern[2+i]);   // Non-zero elements for the i_column short pattern
            for(int j = 0; j < nze; j++) {
                int row_index = (int) (sparsity_pattern[2+(n_col+1)+index+j]);
                if(i <= (row_index-n_col*q)  &&  row_index >= (n_col*q)  &&  row_index < (n_col*(q+1))) {
                    int position = 0;
                    for(int k = 0; k < ((row_index) - (n_col * q)); k++) { // In questo modo ottengo una posizione relativa al constrain in analisi
                        position += k;
                    }
                    position += i;
                    final_values[position] += values[index+j];
                           ...
                           Number* values)
{
  if (values == NULL) {
    ...
    std::vector<long long int> sparsity_pattern_g_jac(sparsity_pattern, sparsity_pattern + (2+n+1+n*m));

    long long int nze_final = sparsity_pattern_g_jac[2+n]; // Number of non-zero elements in the final pattern
    pattern_value_match_constrains_jacobian(sparsity_pattern_g_jac, values1, values, m);

      /*std::cout << "x point is: " << std::endl;
      for (int i = 0; i < n; i++) {
          std::cout << x[i] << " ";
      }
      std::cout << std::endl;
      std::cout << "Before: ";
      print_array(values,15);*/
  }
                       ...
                       Index* jCol, Number* values)
{
  if (values == NULL) {
    ...
    print_array_ind(iRow,10);
    print_array_ind(jCol,10);
