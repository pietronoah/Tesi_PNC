...
bool HS071_NLP::eval_grad_f(Index n, const Number* x, bool new_x, Number* grad_f)
{
  ...
  auto d_f = gradient(f, wrt(p[0], p[1], p[2], p[3]), at(p));
                           ...
                           Number* values)
{
  if (values == NULL) {
    ...
    auto d_g_0 = gradient(g_0, wrt(p[0], p[1], p[2], p[3]), at(p));
    
    auto d_g_1 = gradient(g_1, wrt(p[0], p[1], p[2], p[3]), at(p));
    
    auto d_g_2 = gradient(g_2, wrt(p[0], p[1], p[2], p[3]), at(p));
