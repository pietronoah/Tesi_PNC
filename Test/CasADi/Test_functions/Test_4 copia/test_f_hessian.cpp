...
int main() {
    ...
    //vector<long long int> test = obj_f_hes_sparsity_out(0);
    //int src[] = { 1, 2, 3, 4, 5 };
    //int n = sizeof(sparsity_pattern) / sizeof(sparsity_pattern[0]);
    ...
    double* values3 = (double*) malloc(16*sizeof(double));
    ...
    cout << "Array without zeroes:" << endl;
    print_array(values3, (n*(n+1))/2 );


    double* iRow = (double*) malloc((n*(n+1))/2*sizeof(double));
    double* jCol = (double*) malloc((n*(n+1))/2*sizeof(double));
